...
//        boolean isCornSnack = calcByRegex(cornSnack, "옥수수 스낵", promotion);
        ...
        if (!isJelly && !isEnergyBar && !isProtein && !isChoco && !isCandy && !isCracker
                && !isBasicSnack && !isHotSnack && !isBoxSnack && !isPotatoSnack && !isKidsSnack) {
    ...
    public void calcFOODSubCategory(Promotion promotion) {
        ...
        String iceFoodRegex = "(?i)(만두|교자|피자|닭강정|냉동|치킨|슈마이)";
        String sausageRegex = "(?i)(후랑크|프랑크|핫바|꼬치바|소시지|맥스봉|비엔나|꼬치다|스노우크랩킹" +
                "|오븐구이치킨봉|오븐구이치킨윙|휠터치|크랩|직꾸닭|핫도그|육즙가득|부리또|천하장사)";
        String canFoodRegex = "(?i)(스팸싱글|스팸라이트|스팸클래식|리챔|살코기참치|동원참치|고추참치|야채참치|김치찌개참치)";
        String soupRegex = "(?i)(수프|죽)";
        String healthyFoodRegex = "(?i)(그레인보울|프로틴|닭가슴살|두부|건강|레모나|비타민|치즈|쓹닭|정관장)";
        String riceRegex = "(?i)(햇반|오뚜기밥|흑미밥|잡곡밥|백미밥|현미밥|고시히카리밥|영양밥|약밥|맛있는큰밥|오곡밥)";
        String ramenRegex = "(?i)(라면|너구리|안성탕면|불닭볶음면|짜파게티|마라탕면|무파마|비빔면|배홍동|사리곰탕면|오징어짬뽕" +
                "|진짬뽕|카구리|짜파구리|컵누들|컵쌀국수|큰컵|사발|큰사발|야끼소바불닭|쿠티크투움바파스타|쿠티크매콤물비빔면|짜슐랭" +
                "|치즈게티|우육면|세이면)";
        String tangRegex = "(?i)(찌개|곰탕|설렁탕|삼계탕|추어탕|미역국|무국|장터국|순대국|순댓국|된장국|어묵탕|짜글이)";
        String kimchiRegex = "(?i)(김치|젓갈)";
        String jellyRegex = "(?i)(젤리|쁘띠첼|구미|해피세븐|하리보|탱글탱글)";
        ...
        String hangOverRegex = "(?i)(상쾌환|오징어|케이쿨환|비상대책환|한잔허제간만세|헛깨하니술깨|숙취)";
