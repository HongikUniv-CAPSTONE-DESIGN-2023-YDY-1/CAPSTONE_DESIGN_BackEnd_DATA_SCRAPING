...
public class CrawlingService {
    ...
    public List<Promotion> crawling() {
        ...
        Arrays.stream(brands).parallel()
                .forEach(brand -> IntStream.range(1,5).parallel().forEach(value -> {
                    try {
                        crawling(brand,1,value,-1);
                    } catch (IOException e) {
    ...
    private void crawling(String brand, int page, int category,int lastPage) throws IOException {
        ...
        col_md_6s.parallelStream().forEach(element -> {
            ...
            List<Promotion> promotions = promotionData.textNodes().stream()
                    ...
                    .map(textNode -> new Promotion(promotionData, textNode,category,brand))
                    .collect(Collectors.toList());
            promotions.parallelStream().forEach(promotion -> promotion.setImgUrl(promotionImg));
        ...
        if(page<lastPage){
            crawling(brand,++page,category,lastPage);
        }
    }
